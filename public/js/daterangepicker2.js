...
(function (root, factory) {
    ...
    var DateRangePicker2 = function(element, options, cb) {
        ...
        if (this.locale.firstDay != 0) {
            // 요일 변경 var iterator = this.locale.firstDay;
            var iterator = this.locale.firstDay-1;
    ...
    DateRangePicker2.prototype = {
        ...
        renderCalendar: function(side) {
            ...
            var startDay = daysInLastMonth - dayOfWeek + this.locale.firstDay; //요일 변경 + 1;

          if (startDay > daysInLastMonth)
                startDay -= 7;
            if (dayOfWeek == this.locale.firstDay){
              startDay = daysInLastMonth - 7;
              //요일 변경 startDay = daysInLastMonth - 6;
            }
            ...
            for (var row = 0; row < 6; row++) {
                ...
                for (var col = 0; col < 7; col++) {
                    ...
                    if (calendar[row][col].isoWeekday() > 5){
                      //요일변경 classes.push('weekend');
                      if(calendar[row][col].isoWeekday()==6){
                        classes.push('weekend');
                        classes.push('weekend_sat');
                      }  if(calendar[row][col].isoWeekday()==7){
                        classes.push('weekend');
                        classes.push('weekend_sun');
                      }
                    }

                    // hol = (calendar[row][col].month()+1 >= 10 ? (calendar[row][col].month()+1) : "0"+(calendar[row][col].month()+1)) +""+ (calendar[row][col].date() >= 10 ? calendar[row][col].date() : "0"+calendar[row][col].date());
                    // console.log(hol);
                    // if(hol == "0101" || hol == "0505" || hol == "0815" || hol == "1009" || hol == "0301" || hol == "0606" || hol == "1003" || hol == "1225" )
                    //   classes.push('weekend_sun');


                  // lunarHolidayMap.put("101", "구정"); lunarHolidayMap.put("0102", "구정");
                  // lunarHolidayMap.put("408", "석가탄신일"); lunarHolidayMap.put("0814", "추석");
                  // lunarHolidayMap.put("815", "추석"); lunarHolidayMap.put("0816", "추석");
        ...
        move: function() {
            ...
            if (this.opens == 'left') {
                ...
                var containerLeft = this.element.offset().left - this.element.outerWidth() / 2 - 45 - parentOffset.left;
                // if (containerLeft + containerWidth > $(window).width()) { 변경한 부분
              if (805 >= $(window).width()) {
                    this.container.css({
                        top: containerTop,
                        left: 0,
                    ...
                    this.container.children('.drp-calendar').css({float:'inherit'});
                } else {
                    this.container.css({
                        top: containerTop,
                        left: containerLeft,
                ...
                this.container.children('.drp-calendar').css({float:'left'});
                }
            }
        },
    ...
    $.fn.daterangepicker = function(options, callback) {
        ...
        this.each(function() {
            ...
            el.data('daterangepicker2', new DateRangePicker2(el, implementOptions, callback));
        });
        return this;
    };
