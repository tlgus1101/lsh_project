            ...
            <div class="col-md-12">
              <div class="media probootstrap-media d-flex align-items-stretch mb-4 probootstrap-animate">
                <div class="probootstrap-media-image"
                     style="background-image: url({{ $data->room_rep_image_route.$data->room_rep_image_save_name }})">
                </div>
                <div class="media-body">
                  <h5 class="mb-1 reservation-main">{{ $data->name }}</h5>
                  <div><span class="reservation-title">객실정보</span> <span
                      class="reservation-contents">{{ $data->room_name }}</span>
                  </div>
                  @if( $data->reservation_type == "숙박" )
                    <div class="mb-3"><span class="reservation-title">방문일자</span>
                      <span
                        class="reservation-contents">{{ date("Y년m월d일", strtotime($data->reservation_use_start_date)) }} ~
                  ...
                  @else
                    <div class="mb-3"><span class="reservation-title">방문일자</span>
                      <span
                        class="reservation-contents">{{ date("Y년m월d일 H시", strtotime($data->reservation_use_start_date)) }} ~
                                        @if(date("Y년m월d일", strtotime($data->reservation_use_start_date)) == date("Y년m월d일", strtotime($data->reservation_use_end_date)))
                          {{ date("H시", strtotime($data->reservation_use_end_date)) }}
                        @elseif(date("Y년m월", strtotime($data->reservation_use_start_date)) == date("Y년m월", strtotime($data->reservation_use_end_date)))
                          {{ date("d일 H시", strtotime($data->reservation_use_end_date)) }}
                        @elseif(date("Y년", strtotime($data->reservation_use_start_date)) == date("Y년", strtotime($data->reservation_use_end_date)))
                          {{ date("m월d일 H시", strtotime($data->reservation_use_end_date)) }}
                        @else
                          {{ date("Y년m월d일 H시", strtotime($data->reservation_use_end_date)) }}
                        @endif
                                    </span>
                    </div>
                  @endif
                  <div class="mb-1">
                    <button class="btn btn-primary w-100" onclick="detail({{ $data->reservation_idx }})"
...
{{--                  </div>--}}
                  ...
                  @if (strtotime($data->reservation_use_end_date) > strtotime(date("Y-m-d h:i:s")) && ( $data->reservation_state == "예약완료" || $data->reservation_state == "예약대기" ))
                    <div>
                      <button class="btn btn-danger w-100" onclick="cancle('{{ $data->partner_idx }}','{{ $data->reservation_idx }}')">예약 취소
                      </button>
                    </div>
                  @else
                    <div>
